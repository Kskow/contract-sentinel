...
def _make_rules() -> tuple[list, NestedFieldRule]:
    rules: list = [TypeMismatchRule(), NullabilityMismatchRule(), RequirementMismatchRule()]
...
class TestNestedFieldRule:
    ...
    def test_skips_consumer_sub_fields_absent_from_producer(self) -> None:
        # Missing nested fields are the domain of ConsumerOnlyRule, not here.
        _, nested = _make_rules()
        producer = field(
            name="data",
            type="object",
