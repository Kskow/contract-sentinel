...
# Honour AWS_ENDPOINT_URL from the environment so the same tests work both
# inside Docker Compose (http://localstack:4566) and GitHub Actions (http://localhost:4566).
_ENDPOINT_URL = os.environ.get("AWS_ENDPOINT_URL", "http://localstack:4566")
...
def cli_env(s3_bucket: str) -> dict[str, str]:
    ...
    return {
        ...
        "AWS_ENDPOINT_URL": _ENDPOINT_URL,
